...
class YOLOLoss(nn.Module):
    def __init__(self, num_classes, strides=[8, 16, 32], start_weights=100):
        super().__init__()
    ...
    def get_losses(self, x_shifts, y_shifts, expanded_strides, labels, outputs):
        ...
        for batch_idx in range(outputs.shape[0]):
            ...
            if num_gt == 0:
                ...
                num_fg += num_fg_img
                num_fgs.append(num_fg_img + num_fgs[-1] if len(num_fgs) != 0 else num_fg_img)
        ...
        loss_cls = self.bcewithlog_loss(cls_preds.view(-1, self.num_classes)[fg_masks], cls_targets)
        loss_iou = self.iou_loss(bbox_preds.view(-1, 4)[fg_masks], reg_targets)
        if self.epoch > self.start_weights:
            weights = torch.Tensor()
            weights = weights.to(device)
            loss_iou_detach = loss_iou.detach()
            loss_cls_detach = loss_cls.detach().sum(dim=1)
            weight_factor = 1
            loss_combine = weight_factor * loss_cls_detach + loss_iou_detach
            last_num_fg = 0
            for one_num_gf in num_fgs:
                loss_combine_one_num_fg = loss_combine[last_num_fg:one_num_gf]
                plain_weights = (-loss_combine_one_num_fg).softmax(dim=0)
                _, topk = torch.topk(plain_weights, one_num_gf - last_num_fg)
                threshold = 1 - plain_weights[topk[8]]
                new_weights = plain_weights + threshold
                weights = torch.cat(
                    (weights, new_weights))
                last_num_fg = one_num_gf
            print(weights)
        else:
            weights = torch.ones(int(num_fg))
            weights = weights.to(device)
        weights = weights.clamp(max=2.5)

        loss_cls = loss_cls.sum(dim=1)
        loss_cls = loss_cls * weights
        loss_iou = loss_iou * weights
