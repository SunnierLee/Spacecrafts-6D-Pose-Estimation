...
def get_points2d_error(image_name_, p_, mask_):
    ...
    error_ = np.mean(np.sqrt((gt_p2d_one - p_) ** 2))
    if error_ == np.nan:
        print()
    return error_
...
regression_test = 'heatmap-based/points_by_heatmap_test_with_score_best_bydet_gooddet.txt'
...
for line in regression_lines:
    ...
    linshi = []
    for ps in line[-11:]:
        linshi.append(eval(ps))
    points.append(linshi)
boxes = boxes_det
...
for i in range(len(points)):
    ...
    for j in range(11):
        point[j, 0] = (float(point[j, 0]) - dx) * crop_w / nw + float(left)
        point[j, 1] = (float(point[j, 1]) - dy) * crop_h / nh + float(top)
        # point[j, 0] = (float(point[j, 0]) - dx) * crop_w / nw
        # point[j, 1] = (float(point[j, 1]) - dy) * crop_h / nh
    ...
    #     # cv2.waitKey(0)
    #     for j in range(11):
    #         ax.scatter(int(point[j, 0]), int(point[j, 1]), color=(0, 1, 1))
    #     ax.axis('off')
    #     fig.savefig(r'C:\Users\Administrator\Desktop\python\local_python\Pose Estimation\pic/bad_heatmap_{}'.format(
...
for j in range(len(points)):
    ...
    #     continue
    mask = []
    point = np.array(points[j][:, :2], dtype=np.double).reshape((-1, 2))
    good_poins = []
    t = 0
    for p in points[j]:
        t += 1
    ...
    #         overlay = cv2.circle(overlay, (int(point[k, 0]), int(point[k, 1])), 2, (255, 255, 0), thickness=2)
    #         # cv2.putText(overlay, str(good_poins[k]), (int(point[k, 0]), int(point[k, 1])), cv2.FONT_HERSHEY_PLAIN, 2.0, (0, 255, 255), thickness=2)
    #     alpha = 0.8
    #     ori_image = cv2.addWeighted(overlay, alpha, ori_image, 1-alpha, 0)
    #     cv2.imwrite(r'C:\Users\Administrator\Desktop\python\local_python\Pose Estimation\zhanghaopeng\Pose Estimation\bat_img\heatmap_{}'.format(image_name), ori_image)
    #     # cv2.imshow(image_name, ori_image)
    #     # cv2.waitKey(0)
    if point.shape[0] > 4:
        found, r, t = cv2.solvePnP(point3s_, point, camera.K(), dist, flags=cv2.SOLVEPNP_EPNP)
    else:
        bad_img.append(image_name)
        continue
    if image_name in bad_bydet:
        continue
    points_error.append(get_points2d_error(image_name, point, mask))
    # '''重投影'''
    ...
    #         color=color, linewidth=line_width, alpha=alpha)
    # ax.axis('off')
    # fig.savefig(r'C:\Users\Administrator\Desktop\python\local_python\Pose Estimation\pic/real_heatmap_project_{}'.format(
    #     image_name.replace('jpg', 'pdf')))
    pred_pose.append({'image_name': image_name, 'r': r, 't': t})
    # print(r, t)
print(points_error)
print(sum(points_error))
print(np.mean(points_error))
...
# np.savetxt('position_error_byheatmap.csv', error_positions, delimiter=',')
# np.savetxt('orientation_error_byheatmap.csv', error_orientations, delimiter=',')
