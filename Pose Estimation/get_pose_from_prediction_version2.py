...
for threshold in range(1):
    ...
    for j in range(len(regression_pred)):
        ...
        max_score = max(np.array(list(reg_pred.values())[1:])[:, -1])
        points_3d = []
        points_2d = []
        score = []
        ...
        for i in range(11):
            ...
            # points_2d.append([pred[0]+float(detection_pred[j][0]), pred[1]+float(detection_pred[j][1])])
            points_2d.append([pred[0], pred[1]])
        for i in range(11):
            pred = list(reg_pred.values())[i + 1]
            if pred is None or i in idxs or pred[2] < top_threshold:
                continue
            idxs.append(i)
            score.append(pred[2])
            points_3d.append(point3s[i])
